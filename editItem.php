...
<html lang="en-US">
    ...
    <body class="grid-container">
            ...
            if ($conn = mysqli_connect("localhost","root","")) {
                ...
                if ($stmt = mysqli_prepare($conn, $sql)) {
                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        ...
                        if ($discountedUntil > 0) {
                            $splitDate = str_split($discountedUntil, 2);
                            $dayEnd = $splitDate[3];
                            $monthEnd = $splitDate[2];
                            $yearEnd = $splitDate[0] . $splitDate [1];
                        } else {
                            $dayEnd = 0;
                            $monthEnd = 0;
                            $yearEnd = 0;
        ...
        <form method="POST" action="#" id="addItem" enctype="multipart/form-data">
            <div class="formInput">
                <p>Item name:</p>
                <input class="textArea" type="text" name="itemName" value="<?php echo $title; ?>" required>
            </div>
            <div class="formInput">
                <p>Description</p>
                <input class="textArea" type="text" name="itemDescription" value="<?php echo $description; ?>" required>
            </div>
            <div class="formInput">
                <p>Category</p>
                <select class="textArea" name="category">
                    ...
                    <option value="smartWatches"<?php if ($category = "smartWatches") { echo " selected";} ?>>Smart Watches</option>
                    <option value="smartHome" <?php if ($category = "smartHome") { echo " selected";} ?>>Smart Home</option>
                </select>
            </div>
            <div class="formInput">
                <p>Price</p>
                <input class="textArea" type="text" name="price" value="<?php echo $price; ?>" required>
            </div>
            <div class="formInput">
                <p>Adult item</p>
                <input type="radio" name="adultItem" value="1" <?php if ($adult = "1") { echo " checked";} ?>>
                <label for="yes">yes</label>
                <input type="radio" name="adultItem" value="0" <?php if ($adult = "0") { echo " checked";} ?>>
                <label for="no">no</label>
            </div>
            <div class="formInput">
                <p>On sale?</p>
                <input type="radio" name="discountItem" value="1" <?php if ($discounted = "1") { echo " checked";} ?>>
                <label for="yes">yes</label>
                <input type="radio" name="discountItem" value="0" <?php if ($discounted = "1") { echo " checked";} ?>>
                <label for="no">no</label>
            </div>
            <div class="formInput">
                <p>If so until when? (date mentioned here will be the date the discount ends)</p>
                <label for="discountEndDay">Day:</label>
                <input class="textArea" type="text" name="discountEndDay" value="<?php echo $dayEnd; ?>">
                <label for="discountEndMonth">Month:</label>
                <input class="textArea" type="text" name="discountEndMonth" value="<?php echo $monthEnd; ?>">
                <label for="discountEndYear">Year:</label>
                <input class="textArea" type="text" name="discountEndYear" value="<?php echo $yearEnd; ?>">
            </div>
            <div class="formInput">
                <p>By how much percent?</p>
                <input class="textArea" type="text" name="discountPercent" value="<?php echo $discountedPercentage; ?>">
            </div>
            <div class="formInput">
                <p>Upload main item image here.</p>
                <img src="images/<?php echo $mainImage; ?>">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn = mysqli_connect("localhost","root","")) {
        mysqli_select_db($conn,"webshop");
        //setting the input
        $sql = "UPDATE items SET title = ?, description = ?, category = ?, price = ?, adult = ?, discounted = ?, discountedUntil = ?, discountedPercentage  = ?, mainImage  = ? WHERE id = $id";
